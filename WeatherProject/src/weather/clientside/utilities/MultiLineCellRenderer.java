...
/**
 * This class defines a customized <code>TableCellRenderer</code> with the 
 * specific purpose of allowing multi-line cells inside a <code>JTable</code>.
 * 
 * NOTE: It can only be used for one column in a table at a time and is copied 
 * with tiny changes from http://stackoverflow.com/questions/965023/how-to-wrap-lines-in-a-jtable-cell
 */
public class MultiLineCellRenderer implements TableCellRenderer {
    ...
    class CellArea extends DefaultTableCellRenderer {
        ...
        private CellArea(String s, JTable tab, int row, int column,
                boolean isSelected) {
            text = s;
            rowIndex = row;
            columnIndex = column;
